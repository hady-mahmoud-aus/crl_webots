...
from .logger import episode_dict
...
class RandomPolicy:
    ...
    def addTransition(self, cell_status) -> bool:
        ...
        self.transition['state'] = self.states[0] 
        self.transition['action'] = self.action_code
        self.transition['reward'] = reward
        self.transition['next_state'] = self.states[1] 
        self.transition['done'] = True if revealed or timeout else False
                
        self.buffer.append(deepcopy(self.transition)) 
