...
def readSensors(sensors: dict):
    readings = {}
    for name, sensor in sensors.items():
        readings[name] = round(sensor.getValue(), )
...
def readGPS(gps):
    ...
    reading = [round(value, 2) for value in reading]
    
    return {axis: clean_zero(value) for axis, value in zip(axes, reading)}


def readHeading(inertial_unit):
    theta =  round(inertial_unit.getRollPitchYaw()[2], 2)
    
    return {"sin": clean_zero(sin(theta)), "cos": clean_zero(cos(theta))}

def clean_zero(value, eps=1e-9):
    if abs(value) < eps:
        return 0.0
    return value